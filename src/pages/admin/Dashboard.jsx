...
const ReportsSection = ({ orders }) => {
    ...
    const topPiezas = Object.entries(productRevenue)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
        .map(([nombre, ingreso]) => ({ nombre, ingreso, unidades: productCounts[nombre] || 0 }));
        ...
        <div className="inf">
            ...
            <header className="inf-head">
                <div className="inf-head-texto">
                    ...
                    <p className="inf-sub">
                        {rangoRotulo} · {filtered.length} pedido{filtered.length !== 1 ? 's' : ''}
                    </p>
                </div>
            ...
            <section className="inf-fila-principal">
                ...
                <article className="inf-kpis">
                    <div className="inf-kpi">
                        ...
                        <span className="inf-kpi-s">{paidFiltered.length} cobrados</span>
                    </div>
                    <div className="inf-kpi">
                        <span className="inf-kpi-l">Tasa de pago</span>
                        <span className="inf-kpi-v">{conversionRate} %</span>
                        <span className="inf-kpi-s">{paidFiltered.length} de {filtered.length} pedidos</span>
                    </div>
                    <div className="inf-kpi">
                    ...
                    {ordersByDay.map((d, i) => (
                        ...
                        >
                            <div
                                ...
                                style={{ height: d.count === 0 ? '2px' : `${Math.round((d.count / maxDayCount) * 100)}%` }}
                            />
                        </div>
                    ))}
                </div>
                <div className="inf-grafica-pie">
                    <span>{ordersByDay[0]?.label}</span>
                    <span>{ordersByDay[ordersByDay.length - 1]?.label}</span>
                </div>
            </section>
                    ...
                    {sourceEntries.length === 0 ? (
                        ...
                        <div key={src} className="inf-canal">
                            <div className="inf-pieza-fila">
                                ...
                                <span className="inf-canal-meta">{n} pedido{n !== 1 ? 's' : ''}</span>
                            </div>
